...
The core deliberately has no renderer, audio, input, UI-framework, filesystem, or platform dependency. `settings-adapters` also never executes domain behavior; it only registers ordinary core definitions/capabilities and translates setting changes into command types owned by consumers.
